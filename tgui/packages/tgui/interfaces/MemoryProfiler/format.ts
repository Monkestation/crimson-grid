...
/**
 * `/datum/reagents::reagent_list` - a group row's two name fields, joined.
 *
 * The extension used to ship this pre-joined as an `owner` field and stopped:
 * it was a third string per row saying nothing the two fields beside it do not,
 * minted for every one of the thousands of type vars a world has, on every
 * export. Joined here instead, in one place, so the two tables that show it
 * cannot end up spelling it differently.
 */
export function varPair(row: { typepath: string; var: string }): string {
