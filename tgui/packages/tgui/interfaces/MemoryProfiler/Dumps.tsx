  ...
  return (
    <Section title="File dumps">
      <Stack vertical>
        ...
        <Stack.Item>
          {/* Not a footnote: someone who opens an "all" dump expecting the
              biggest list on line 2 reads the first screenful as if it were the
              answer. A numbered dump keeps the ranking, which makes the
              difference between the two buttons invisible without this. */}
          <NoticeBox info>
            An <b>all</b> dump comes out in list-number order, not biggest first
            - ranking it would mean holding a row per list, which is the
            allocation a file dump exists to avoid. Sort it afterwards on the
            bytes column. A numbered dump is still biggest first.
          </NoticeBox>
        </Stack.Item>
        <Stack.Item>
