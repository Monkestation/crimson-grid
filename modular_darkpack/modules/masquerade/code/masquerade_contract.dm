...
/obj/item/masquerade_contract/attack_self(mob/user, modifiers)
	...
	for(var/mob/living/carbon/breacher in GLOB.masquerade_breakers_list)
		...
		to_chat(user, span_info("[breacher.real_name], Masquerade Breaches: [5 - breacher.masquerade_score], Diablerist: [(HAS_TRAIT(breacher, TRAIT_DIABLERIE) && !HAS_TRAIT(breacher, TRAIT_HIDDEN_DIABLERIE)) ? "<b>YES</b>" : "NO"], [location_info]"))
