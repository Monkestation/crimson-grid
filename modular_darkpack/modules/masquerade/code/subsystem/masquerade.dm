...
/datum/controller/subsystem/masquerade/proc/masquerade_reinforce(atom/source, mob/living/player_breacher, reason)
	...
	for(var/masquerade_breach in masquerade_breachers)
		...
		if(source_matches || reason == "death of breacher" || reason == "debug")
			if(!reason || (reason in masquerade_breach) || (reason == MASQUERADE_REASON_PREFERENCES) || (reason == "debug") || (reason == "death of breacher"))
